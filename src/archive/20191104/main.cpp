...
int main() {
  ...
  const double MAX_VEL = 21.9; // reference max velocity
    ...
    if (length && length > 2 && data[0] == '4' && data[1] == '2') {
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
		  ...
		  // Ego state default
		  if (Ego.transition==true && Ego.d > Ego.lane*4+1.6 && Ego.d < Ego.lane*4+2.4) {
		  ...
		  for (int i=0;i<sensor_fusion.size();i++) {
			  ...
			  if (bumper_dis < 90 && bumper_dis > -15) {
				  ...
				  if (car[5] > car_s && bumper_dis < 18) {
					  if (car[6] > (Ego.lane*4-0.5) && car[6] < (Ego.lane*4+4.5)) {
						  ...
						  if (speed < Ego.ref_vel) {
							  ...
							  if (bumper_dis < 14) {
								  Ego.ref_vel = speed * bumper_dis / 14;
							  } else Ego.ref_vel = speed;
							  Ego.num_path_p = std::min(bumper_dis / del_speed / ref_t/5+SHORT_PATH_PTS,50.0+SHORT_PATH_PTS);
							  Ego.state = "KL";
						  }
					  }
				  }
