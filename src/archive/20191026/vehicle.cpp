...
vector<Vehicle> Vehicle::keep_lane_trajectory(map<int, vector<Vehicle>> &predictions) {
  ...
  vector<Vehicle> trajectory = {Vehicle(lane, this->vx, this->vy, this->s, this->d, state)};
  ...
  trajectory.push_back(Vehicle(lane, this->vx, this->vy, this->s, this->d, "KL"));
...
vector<Vehicle> Vehicle::generate_predictions(const vector<double> &maps_s, 
  ...
  for(int i = 0; i < horizon; ++i) {
	  ...
	  double new_d = this->d - sin(dir_v - dir_road) * vel * (i+1);
    ...
    predictions.push_back(Vehicle(original_lane, this->vx, this->vy, this->s + vel*(i+1), new_d, new_state));
  }
