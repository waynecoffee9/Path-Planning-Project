...
const double DEL_DIST = 12.0; // gap between two vehicles for Ego to advance
const double FRONT_DIST = 40.0; // distance of vehicle in front of Ego
// distance (clearance) of vehicles in front and back (at target lane) for Ego to make lane change.
const vector<double> FRONT_BACK_TARGET_DIS = {14.0, 10.0}; 
const double FRONT_BACK_DIS = 8.0; // distance (clearance) of vehicles in front and back for Ego to make lane change.
...
float calculate_cost(Vehicle &Ego, const Vehicle &trajectory,
  ...
  if (state == "KL") {
	  ...
	  if (del_dist > DEL_DIST && closest_vehicle_list[Ego.lane][1] < FRONT_DIST
	  ...
	  for (map<int, vector<Vehicle>>::iterator it = predictions.begin(); it != predictions.end(); ++it) {
		  ...
		  if ((it->second[0].lane + lane_change) == target_lane) {
			  if (it->second[0].s > (Ego.s - FRONT_BACK_TARGET_DIS[1]) && it->second[0].s < (Ego.s + FRONT_BACK_TARGET_DIS[0])) {
				  cost = SAFETY;
			  }
			  if (it->second[2].s > (target_s - FRONT_BACK_TARGET_DIS[1]) && it->second[2].s < (target_s + FRONT_BACK_TARGET_DIS[0])) {
				  cost = SAFETY;
			  }
		  }
		  // check for other vehicles in the same current lane as Ego.
		  if (it->second[0].lane == Ego.lane) {
			  if (it->second[2].s > (target_s - FRONT_BACK_DIS) && it->second[2].s < (target_s + FRONT_BACK_DIS)) {
				  cost = SAFETY;
			  }
		  }
	  }
